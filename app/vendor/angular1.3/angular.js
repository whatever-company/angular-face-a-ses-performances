  ...
  this.$get = ['$injector', '$exceptionHandler', '$parse', '$browser',
    ...
    function beginPhase(phase) {
      ...
      if ($rootScope.$$phase === '$digest') {
        digestPhase = window.performance.now();
      }
    }

    function clearPhase() {
      if ($rootScope.$$phase === '$digest') {
        window.angular._digest = window.performance.now() - digestPhase;
        window.angular._maxDigest = Math.max(window.angular._maxDigest || 0, window.angular._digest);
      }
